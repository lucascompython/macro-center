			...
			{#each variables as variable (variable.id)}
				<div class="variable-row">
					<input
						...
						type="text"
						value={variable.name}
						oninput={(event) => updateVariable(variable.id, { name: event.currentTarget.value })}
					/>
					...
					<select
						...
						onchange={(event) =>
							updateVariable(variable.id, { scope: event.currentTarget.value as VariableScope })}
					>
					...
					<button class="delete-btn" onclick={() => deleteVariable(variable.id)} title="Delete variable">
						Delete
					</button>
				</div>
			{/each}
...
<style>
	...
	.variable-row {
		display: grid;
		grid-template-columns: 1fr 92px 92px 1fr 1fr auto;
		gap: 0.35rem;
	}
