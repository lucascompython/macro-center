<script lang="ts">
    ...
    const renameSubflowDefinition: RenameSubflow = (definitionId, rawName) => {
        ...
        for (const node of nodes) {
            ...
            if (
                (node.type === "subflowGroupNode" ||
                    node.type === "submacroNode") &&
    ...
    async function stopRecording() {
        recordingError = "";
        try {
            const recorded = await invoke<RecordedMacro>(
    ...
    function selectedDelayBaselines() {
        ...
        for (const node of nodes) {
            if (selectedIds.has(node.id) && node.type === "delayNode") {
                baselines[node.id] = Math.max(
                    0,
                    Number(node.data.delay ?? 1000) || 0,
                );
            }
        }
    ...
    function deleteSelectedDelayNodes() {
        const delayIds = new Set(
            Object.keys(contextMenu?.delayBaselines ?? {}),
        );
        ...
        function reachableOutputs(
            ...
            for (const edge of outgoing) {
                if (delayIds.has(edge.target)) {
                    for (const output of reachableOutputs(
                        edge.target,
                        visited,
                    )) {
                        outputs.push(output);
        ...
        for (const incomingEdge of triggerEdges) {
            if (
                !delayIds.has(incomingEdge.target) ||
                delayIds.has(incomingEdge.source)
            )
                continue;
    ...
    function optimizeTypingTextSelection() {
        ...
        const edgeKeys = new Set<string>();
        for (const edge of edges) {
            if (consumedIds.has(edge.source) || consumedIds.has(edge.target))
                continue;
            keptEdges.push(edge);
            edgeKeys.add(
                edgeKey(
    ...
    function parseTypingRun(
        ...
        return {
            ...
            nodes: consumedNodes,
            consumedIds,
            incomingEdges: unconsumedEdges(
                incomingByTarget.get(firstNode.id),
                consumedIds,
                "source",
            ),
            outgoingEdges: unconsumedEdges(
                outgoingBySource.get(lastNode.id),
                consumedIds,
                "target",
            ),
        };
    ...
    function nextPrintableKeyNode(
        ...
        while (nextNodeId) {
            ...
            if (nextNode.type === "delayNode") {
                ...
                nextNodeId = singleOutgoingTarget(
                    nextNode.id,
                    outgoingBySource,
                );
                continue;
            ...
            return nextNode.type === "keyNode" &&
                textForKeyNode(nextNode) !== undefined
    ...
    function addReplacementEdge(
        ...
        replacementEdges.push(
            makeGraphEdge({ source, sourceHandle, target, targetHandle }),
        );
    }
    ...
    onMount(() => {
        ...
        void listenEvent<MousePositionPayload>(
            "mouse_position",
            ({ payload }) => {
    ...
    function nodeDimension(
        ...
        if (measured.measured?.[property])
            return measured.measured[property] ?? 0;
    ...
    function filteredEdgeDropTemplates() {
        ...
        for (const template of nodeTemplates) {
            const searchable =
                `${template.label} ${template.category}`.toLowerCase();
            if (searchable.includes(query))
                cachedEdgeDropTemplates.push(template);
        }
        return cachedEdgeDropTemplates;
    }
...
>
    ...
    >
        ...
        <Panel position="top-right">
                ...
                {#if showRecordOptions && !isRecording}
                    <div class="record-menu" role="menu">
                        <button
                            role="menuitem"
                            onclick={() =>
                                startRecordingWithMode("movesBeforeClicks")}
                        >
                            Clicks Only
                        </button>
                        <button
            ...
            <button
                class="panel-btn"
                onclick={() =>
                    (showMousePositionPanel = !showMousePositionPanel)}
            >
                Mouse
            </button>
            <button
            ...
            {#if showMousePositionPanel}
                <div class="mouse-position-panel">
                    <div class="mouse-position-header">
                        <div>
                            <div class="mouse-position-title">
                                Mouse Position
                            </div>
                            <div
                                class="mouse-position-status"
                                class:active={isMousePositionMonitoring}
                            >
                                {isMousePositionMonitoring
                                    ? "Listening"
                                    : "Stopped"}
                            </div>
                        </div>
                        <button
                            class="panel-btn"
                            onclick={toggleMousePositionMonitoring}
                        >
                    ...
                    <div class="mouse-click-header">
                        <span>Clicks</span>
                        <button
                            onclick={clearSavedMouseClicks}
                            disabled={savedMouseClicks.length === 0}
                        >
                            Clear
                        </button>
                    </div>
            ...
            {#if submacros.length > 0}
                <div class="submacro-panel">
                    ...
                    {#each submacros as submacro (submacro.id)}
                        <button
                            class="submacro-btn"
                            onclick={() => addSubmacroInstance(submacro)}
                        >
                            {submacro.name}
                        </button>
                    {/each}
    ...
    {#if contextMenu}
        ...
        >
            <button
                role="menuitem"
                onclick={openNodeCreationMenuFromContextMenu}>New Node</button
            >
            ...
            >
                ...
                <input
                    ...
                    oninput={(event) =>
                        applyDelayMultiplier(Number(event.currentTarget.value))}
                />
            ...
            <input
                ...
                onkeydown={(event) => {
                    if (event.key === "Escape") {
                        ...
                        if (firstTemplate)
                            createNodeFromEdgeDrop(firstTemplate);
                    }
                }}
            />
        ...
        <div class="recording-dialog-backdrop">
            ...
            >
                ...
                <button
                    ...
                    onblur={() => {
                        if (recordingShortcut) {
                            recordingShortcut = false;
                            recordedShortcut = shortcutReady()
                                ? recordedShortcut
                                : "";
                        }
                    }}
                ...
                <div class="dialog-actions">
                    <button class="panel-btn" onclick={discardRecordedMacro}
                        >Discard</button
                    >
                    <button
                        class="panel-btn"
